...
router.post('/add', function(req, res, next) {    
    ...
    fs.writeFile(imgsrc, buffer, function(err,written){
     ...
     const imageUrl = `${req.protocol}://${req.hostname}:4000/public/${filename}`;

    if(name.length === 0 || email.length === 0 || city.length === 0 || mobile.length === 0 || file.length === 0
    ...
    if(!errors) {
        ...
        // insert query
        dbConn.query('INSERT INTO students (name,email,city,mobile,image) VALUES(?,?,?,?,?)',
         [name,email,city,mobile,imageUrl], function(err, result) {
            //if(err) throw err
            if (err) {
                req.flash('error', err)
